...
class EndpointExecutor:
    ...
    def _effective_action_timeout_seconds(
        ...
        aid = str(action_id or "").strip().lower()
        requested_aid = str((context or {}).get("action_id") or "").strip().lower()
		  ...
		  for ($i = 0; $i -lt $max; $i++) {
			...
			try {
			  C2F-Status "START"
				    ...
				    if ($runError.Trim()) {
				      if ($errOut.Trim()) {
				        $errOut = $errOut + [Environment]::NewLine + $runError
				      } else {
				        $errOut = $runError
				      }
				    }
...
""".strip()
    ...
    def _execute_windows_script_task(
        ...
        *,
        ...
        if not safe_agent:
            safe_agent = "agent"
        requested_action = str(action_id or "").strip().lower()
        task_time_limit_seconds = max(900, int(timeout_seconds or 120) + 600)
        if requested_action == "global-shell" and str(script_action_id or "").strip().lower() == "custom-os-command":
            task_time_limit_seconds = max(330, int(timeout_seconds or 300) + 30)
        ...
        if result_obj is None:
            ...
            if (
                requested_action == "global-shell"
                and str(script_action_id or "").strip().lower() == "custom-os-command"
            ):
                stop_script = (
                    "$ErrorActionPreference='SilentlyContinue';"
                    "$ProgressPreference='SilentlyContinue';"
                    ...
                    "New-Item -ItemType Directory -Path $dir -Force | Out-Null;"
                    "$kill=Join-Path $dir ('kill-'+$exec+'.flag');"
    ...
    def _build_linux_script(
        ...
        if aid == "custom-os-command":
            ...
            inner = (
                ...
                "out=$(c2f_sudo_exec bash -lc \"$cmd\" 2>&1); rc=$?; "
                "if [ \"$rc\" -ne 0 ] && [ -z \"$(printf '%s' \"$out\" | tr -d '[:space:]')\" ]; then out='custom-os-command requires sudo privileges. Configure endpoint sudo or connector password.'; fi; "

...
export default function PatchWorkbenchShellMin({
  mode = "shell",
  modeOptions = MIN_WORKBENCH_MODES,
  modeSummary = {},
  onModeChange,
  modePane = null,
  renderModePane = null,
      ...
      <div className="card patch-workbench-switcher-card">
        ...
        <div className="page-actions">
          <select
            ...
            onChange={(event) => {
              if (typeof onModeChange === "function") onModeChange(event.target.value);
            }}
          >
            {(Array.isArray(modeOptions) && modeOptions.length ? modeOptions : MIN_WORKBENCH_MODES).map((item) => (
        ...
        <div className="meta-line">
          {modeSummary?.[normalizedMode] || "Choose the workspace mode for this run."}
        </div>
      </div>

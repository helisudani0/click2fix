...
function New-ComposeProjectNameVariant {
  param([string]$Current)

  $base = [string]$Current
  if ([string]::IsNullOrWhiteSpace($base)) { $base = "click2fix" }
  $base = ($base.ToLowerInvariant() -replace '[^a-z0-9_-]', '').Trim()
  if ([string]::IsNullOrWhiteSpace($base)) { $base = "click2fix" }
  if ($base.Length -gt 36) { $base = $base.Substring(0, 36).TrimEnd('-', '_') }
  if ([string]::IsNullOrWhiteSpace($base)) { $base = "click2fix" }
  ...
  for ($attempt = 0; $attempt -lt 20; $attempt++) {
    ...
    $existing = @(Get-ProjectContainerIds -ProjectName $candidate -IncludeAll)
...
function Assert-NoDeadProjectContainers {
  ...
  $allowProjectRotation = $true
  if ($script:composeEnvPath -and (Test-Path $script:composeEnvPath)) {
    $rawRotate = Get-EnvValue -Path $script:composeEnvPath -Key "C2F_INSTALLER_ROTATE_PROJECT_ON_DEAD"
    if (-not [string]::IsNullOrWhiteSpace($rawRotate)) {
      $allowProjectRotation = To-Bool $rawRotate
    }
  }
  $runningContainers = @(Get-ProjectContainerIds -ProjectName $ProjectName)
  if (
    $allowProjectRotation -and
    $runningContainers.Count -eq 0 -and
    $script:composeProjectName -and
    $ProjectName -eq $script:composeProjectName -and
    $script:composeEnvPath -and
    (Test-Path $script:composeEnvPath)
  ) {
    $newProjectName = New-ComposeProjectNameVariant -Current $ProjectName
    Set-EnvValue -Path $script:composeEnvPath -Key "COMPOSE_PROJECT_NAME" -Value $newProjectName
    $script:composeProjectName = $newProjectName
    Write-Host ("Docker still reports dead metadata for project '" + $ProjectName + "'.") -ForegroundColor Yellow
    Write-Host ("Switching installer to fresh compose project '" + $newProjectName + "' and continuing setup.") -ForegroundColor Yellow
    return
  }
  ...
  try {
    ...
    foreach ($containerId in $containerIds) {
      try {
        ...
        # Docker Desktop can temporarily report "No such container" for dead metadata.
    ...
    try {
      ...
      # Network may already be absent; ignore.
    }
  } finally {
...
$scriptDir = Split-Path -Parent $MyInvocation.MyCommand.Path
$envTemplate = Join-Path $scriptDir ".env.patch-workbench.template"
...
Write-Host "== Click2Fix Patch Workbench Setup (Windows) ==" -ForegroundColor Cyan
...
if ([string]::IsNullOrWhiteSpace($backendImage) -or [string]::IsNullOrWhiteSpace($frontendImage) -or [string]::IsNullOrWhiteSpace($imageTag)) {
  throw "Image configuration is missing in .env.patch-workbench. Expected C2F_BACKEND_IMAGE, C2F_FRONTEND_IMAGE, C2F_IMAGE_TAG."
}
...
Write-Host ""
Write-Host "Patch Workbench appliance is ready." -ForegroundColor Green
